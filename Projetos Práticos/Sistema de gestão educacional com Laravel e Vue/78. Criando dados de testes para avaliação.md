...
Começaremos criando o **ModelFactory**, portanto abra o arquivo **/database/factories/ModelFactory.php**.
...
A avaliação está sendo criada com uma data inicial atual e com data final somando uma hora da inicial. Estas datas são fictícias para que tenhamos um controle à partir do momento da criação. Existe também o nome da avaliação que terá uma sentença com 3 palavras.
...
Agora crie a seeder, que será responsável por gerar os conteúdos, rodando o comando abaixo:
...
O código acima mostra como ficou o arquivo ClassTestsTableSeeder.php inteiro. Porém vamos comentar função a função para que você consiga entender a lógica.
...
Começamos guardando instâncias para utilizar dentro das factorys. Guardamos um objeto com todos os ensinos, depois guardamos um objeto com o nosso professor teste, que tem o id igual a um e depois guardamos uma instância da própria classe.
...
Depois de preparar os objetos acionamos a factory para gerar 30 avaliações, porém ao invés de utilizar o método create utilizamos o método **make**, que só prepara o objeto.

Tivemos que utilizar o make porque temos que fazer vários relacionamentos antes de criar a avaliação de fato.

Depois de criar a instância nós guardamos todas os ensinos relacionados ao professor teste e pegamos apenas um ensino aleatório com o método random.
...
Em seguida associamos a avaliação ao ensino. Vejam que estamos utilizando o método **classTeaching**, do model, atrelado ao método **associate**, por isso tivemos que pegar apenas um ensino de cada vez.

Logo depois salvamos a avaliação e chamamos o próximo método **createQuestions**, passando a avaliação como parâmetro.

O mesmo procedimento foi executado depois, porém para professores aleatórios, criando mais cem avaliações aleatórias.
...
Tiramos a particularidade de ser o professor teste com id igual a um, mas sim qualquer outro professor aleatóriamente. O restande do processo é o mesmo.
...
Depois de fazer a criação das questões chamamos o método **createChoices**, passando a coleção de questões criadas. Este método criará as respostas para cada questão.
...
Vejam que para cada questão criada estamos criando 4 respostas e passando o id da questão para ser feito o relacionamento.

O método também pega sempre a primeira resposta e atribui ao campo **true** o valor true realmente. Isso quer dizer que em nossos testes a primeira resposta, de cada pergunta, sempre será verdadeira e o restante falso.

### Rodando testes

Depois de criar todo ambiente de teste, precisamos rodar o comando para atualizar os dados.

Abra o arquivo **/database/seeds/DatabaseSeeder.php** e adicione a chamada para nossa nova seeder.
...
Depois abra o terminal e rode o seguinte comando:
...
Com este último comando você terá as trinta avaliações que criamos e caso queira brincar com as questões basta pegar o a primeira avaliação e consultar. Veja os comandos para executar no tinker.
...
Agora temos toda estrutura de testes criada e podemos começar a desenvolver de fato. Caso você não crie estes dados de teste você pode ter problemas durante o desenvolvimento.

Então uma dica que damos é sempre criar a modelagem e já criar dados de teste. E se possível ainda confira todos os testes para ver se você terá tudo funcionando corretamente. Somente depois disso comece a desenvolver de fato.