...
Desta forma pode verificar que os registros de migrations, da aplicação, estão sendo controlados via banco de dados, pelo Laravel.
...
Com estes comandos, de migrações, nós podemos controlar, de fato, as versões dos bancos de dados. Nós não precisamos mais criar um arquivo **.sql** para criar o backup do banco de dados da aplicação, porque os backups serão as migrations criadas. Cadas vez que executada ela criará a cópia fiel do banco de dados que necessita para sua aplicação. Basta ir desenvolvendo sua aplicação, criando suas migrations e quando for subir para o servidor, no ambiente de produção, terá somente que rodar o comando `$ php artisan migrate` e já terá seu banco de dados inteiro montado.
